...
class AgentNewsSearchTests(unittest.TestCase):
    ...
    def test_search_news_is_available_without_a_connected_account(self) -> None:
        tools = {tool["name"]: tool for tool in tool_definitions({})}

        self.assertIn("search_news", tools)
        self.assertNotIn("UNAVAILABLE RIGHT NOW", tools["search_news"]["description"])
        self.assertIn("the news on a topic", tools["schedule_task"]["description"])
    ...
    def test_list_mode_exposes_only_five_titles_and_dates_to_the_reply(self) -> None:
        ...
        with mock.patch("packages.infrastructure.agent_loop.search_news", return_value=search_result):
        ...
        self.assertEqual(result.tool_calls[0]["name"], "search_news")
    ...
    def test_a_search_that_runs_out_of_time_says_it_took_too_long(self) -> None:
        ...
        with mock.patch(
            "packages.infrastructure.agent_loop.search_news",
    ...
    def test_agent_rules_make_news_lists_compact_and_schedulable(self) -> None:
        self.assertIn("exactly one numbered line per result containing only its title and date", AGENT_LOOP_INSTRUCTIONS)
        self.assertIn("A recurring request to search or watch the web or the news is a standing action", AGENT_LOOP_INSTRUCTIONS)
