...
class PortalStaticPageTests(unittest.TestCase):
    ...
    def test_monitor_actions_support_manual_runs_and_manual_only_mode(self) -> None:
        ...
        self.assertIn("Manual mode", script)
        self.assertIn("No background checks. Use Run now whenever you want a fresh top-five summary.", script)
        ...
        self.assertIn('item.addEventListener("click", (event) => {', script)
        self.assertIn("selectScheduledAction(action.id)", script)
        self.assertIn("state.selectedScheduledActionId = String(actionId || \"\");", script)
        self.assertIn("selectScheduledAction(scheduledActionButton.dataset.agentScheduledActionId || \"\")", script)
