...
class PortalStaticPageTests(unittest.TestCase):
    ...
    def test_about_contact_modal_includes_mobile_keyboard_layout_hooks(self) -> None:
        ...
        self.assertIn("--contact-mobile-composer-height", body)
        self.assertIn("--contact-mobile-status-height", body)
        self.assertIn("--contact-mobile-header-height", body)
        self.assertIn("--contact-mobile-chat-height", body)
        self.assertIn("--contact-mobile-keyboard-transition", body)
        self.assertIn("grid-template-rows: minmax(0, var(--contact-mobile-chat-height)) 0 0", body)
        ...
        self.assertIn("syncContactControlHeights", body)
        self.assertIn("contactMobileVisibleHeight = visibleHeight", body)
        self.assertIn('.contact-modal[data-input-focused="true"] .contact-chat-thread > .contact-message:first-child', body)
        ...
        self.assertIn("height: var(--contact-mobile-viewport-height)", body)
        self.assertIn("height: var(--contact-mobile-chat-height)", body)
        self.assertNotIn("height: var(--contact-mobile-visible-height)", body)
        self.assertIn("bottom: var(--contact-keyboard-offset)", body)
        self.assertIn("bottom: calc(var(--contact-keyboard-offset) + var(--contact-mobile-composer-height))", body)
