...
def build_answer_prompt(
    *,
    ...
    return (
        ...
        "it.\n"
        "dateRangeAssumed, when it is there, means the words naming the period could not be placed, so "
        "the lookup read those days instead. Name the days you are answering about, in a few words, so "
        "the reader can see what was checked and correct it if it was not what they meant.\n"
        "daysNotChecked, when it is there, means the question covered more days than the lookup read. "
        "Say so and name the day it read up to in checkedThrough, because the free time below it is the "
        "answer for part of what was asked and not for all of it.\n"
        ...
        "overlaps another, or an insurancePotentialClaim on a receipt - say it in one line at the end. An "
        "insurancePotentialClaim is screening, not guaranteed coverage: name the policy, deductible, deadline estimate "
        "and source status it supplies, and keep summary-only matches explicitly provisional. Only when it is really there in the records, "
