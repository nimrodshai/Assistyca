...
- Scheduled assistant notifications always use the Assistyca sender credentials from the backend environment. They send `WHATSAPP_SCHEDULED_NOTIFICATION_TEMPLATE_NAME` (default `notification_message`) in `WHATSAPP_SCHEDULED_NOTIFICATION_TEMPLATE_LANGUAGE` (default `en`; the template is registered at Meta as plain English, and `en_US` is a different template that does not exist) and fill the template's `{{1}}` body variable with the requested message.
